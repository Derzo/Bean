...
class Featherer():
    ...
    def feather(self, value):
        if value <= 0:
            ...
            self.i += 1
            if self.i >= int(1 / value):
                self.i = 0
                return True
            else:
                return False

def estimateTimeTo(pos, vel, p):
    d = length(pos-p)
...
class Bean(BaseAgent):
    ...
    def findTargetPoint(self):
        ...
        if ball_prediction is not None:
            for i in range(0, ball_prediction.num_slices):
                ...
                carT = estimateTimeTo(self.pos, self.vel, location)
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        facingTarget = dot(self.mat[1], normalize(target-self.pos))
        velFacingTarget = dot(normalize(self.vel), normalize(target-self.pos))
        velTowardsTarget = 0
        if velFacingTarget > 0:
            velTowardsTarget = length(self.vel) * pow(velFacingTarget, 2)
        

        dodgeVel = self.vel + 500*normalize(target-self.pos)
        dodgeVel = normalize(dodgeVel)*clamp(length(dodgeVel), 0, 2300)
        posAfterDodge = self.pos + dodgeVel*1.2
        ballPosAfterDodge = self.ballPosAt(1.2)

        dodgeTimeGain = estimateTimeTo(self.pos, self.vel, self.ballPos) - self.estimateTimeTo(posAfterDodge, self.vel, ballPosAfterDodge)
        print(dodgeTimeGain)
        # shouldDodgeToGainSpeedGround = velTowardsTarget > 1100 and velTowardsTarget < 2000
        # shouldDodgeToGainSpeedAir = velTowardsTarget > 1000 and velTowardsTarget < 2100
        # turningVel = abs(dot(self.angularVel, self.mat[2]))
        # if shouldDodgeToGainSpeedGround and self.onGroundStable and turningVel < 1:
        ...
        if length(self.vel) < 1000:
            self.controllerState.boost = True


        if facingTarget < 0.5 and length(self.vel)>800:
            self.controllerState.handbrake = True
            # print(length(self.vel))
